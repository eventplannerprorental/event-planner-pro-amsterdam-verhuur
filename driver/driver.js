...
(function(){
  ...
  var LS_DEVICE_USER='epp-amsterdam-driver-device-user-v1';
  ...
  function renderLogin(){
    ...
    if(boundUser){
      ...
      if(pickerRow)pickerRow.classList.remove('hidden');
  ...
  function modal(html){var m=document.createElement('div');m.className='modal';m.innerHTML='<div class="modal-card"><div class="modal-topbar"><button class="modal-back" type="button" data-close>← Terug</button></div><div class="modal-body">'+html+'</div></div>';document.body.appendChild(m);m.querySelectorAll('[data-close]').forEach(function(b){b.onclick=function(){m.remove()}});return m}
  ...
  function offer(o){var m=modal('<h2>Offerte</h2><pre>'+esc(orderText(o))+'</pre><button class="action-btn green" id="waOffer">WhatsApp</button><button class="action-btn blue" id="copyOffer">Kopieer / deel</button>');m.querySelector('#waOffer').onclick=function(){whatsapp(o)};m.querySelector('#copyOffer').onclick=function(){shareText(orderText(o))}}
